  ...
  <div class="container mt-4">
    ...
    <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST">
      ...
      <div class="form-group mb-3">
        ...
        <input class="form-control" id="name" name="name" type="text" value="{{ old('name', $menu->name) }}"
          required>
      </div>

      <div class="form-group mb-3">
        <label for="slug">Slug</label>
        <input class="form-control" id="slug" name="slug" type="text" value="{{ old('slug', $menu->slug) }}"
          readonly required>
      </div>
          ...
          @foreach ($categories as $category)
            <div class="col-md-4">
              <div class="form-check">
                <input class="form-check-input category-checkbox" {{ $menu->categoryCheck($category->id) === true ? 'checked' : '' }} id="cat_{{ $category->id }}"
